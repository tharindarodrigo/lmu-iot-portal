...
The current platform deployment is Docker-based, using containerized services for operational consistency across environments. This keeps hosting practical for the present scope while preserving a clear migration path to orchestration-based scaling later.
...
*Figure 1: End-to-end platform architecture showing telemetry ingress, processing, automation, control dispatch, reporting, and visualization flows.*
...
### All-in-One Platform Positioning (Balanced Comparison)
...
*Figure 2: Comparison of originally proposed toolchain architecture and final integrated platform architecture.*
...
### ESP32 Firmware Architecture: Common Runtime + Device Modules
...
*Figure 3: Automation DAG for converting meter telemetry into an RGB alert command using query and condition nodes.*
...
*Figure 4: Control dashboard view showing command composition, message flow, and command/state lifecycle records.*
...
### Single-Phase Meter Firmware Module (PZEM RS485)
...
### Ingestion, Automation, and Control Logic
...
*Figure 5: Energy dashboard showing historical and realtime telemetry widgets, including line, bar, and gauge visualizations.*
...
*Figure 6: Report pipeline interface showing queue state, run history, and downloadable CSV outputs.*

This architecture turns telemetry into both operational feedback and auditable analytical artefacts.

### Current Deployment (Docker) and Scaling Path
The implementation is currently hosted with Docker containers for the application and supporting services. This deployment model is sufficient for the present workload and keeps operations simple for development and demonstration.

Because the services are already containerized and contract-driven, scaling can be improved in a future phase by moving from single-host Docker operation to an orchestrated multi-node runtime.
...
### Methodology Note on AI-Assisted Development
...
Overall, the platform provides a practical Docker-hosted baseline for energy IoT operations where traceability, interoperability, and operational simplicity are priorities.
...
5. Expand operational observability and deployment runbooks for multi-organization growth.

## Improvements
1. Plan a migration path from single-host Docker deployment to Kubernetes orchestration for horizontal scaling and workload isolation.
2. Define tenant-level resource policies and autoscaling guardrails as part of that scaling phase.
3. Expand validation from correctness-focused checks to include end-to-end performance benchmarks under sustained load.
4. Implement controlled meter write operations (address, alarm threshold, and reset) behind explicit authorization and auditing.
5. Introduce a staged security and compliance hardening plan covering device identity controls, key rotation, and operational monitoring.
6. Replace appendix evidence-only artefacts with linked vendor-primary documentation snapshots for stronger traceability.
...
ManualsLib (n.d.) *Morning PZEM-014 manual (PZEM-014/016 AC communication module)*. Available at: https://www.manualslib.com/manual/3810945/Morning-Pzem-014.html (Accessed: 28 February 2026).

Peacefair (n.d.) *PZEM-016 AC communication module product page*. Available at: https://en.peacefair.cn/product/771.html (Accessed: 28 February 2026).
...
To avoid report bloat, the full firmware source is referenced rather than embedded.

- Local template path: `plan/DeviceControlArchitecture/esp32-rgb-light/esp-32-rgb-light.ino`
- GitHub reference: https://github.com/tharindarodrigo/lmu-iot-portal/blob/main/plan/DeviceControlArchitecture/esp32-rgb-light/esp-32-rgb-light.ino
- Portal note: the same firmware template can be retrieved from the portal firmware template view.

![RGB LED actuator hardware used for device-control validation.](report-images/rgb-led.jpeg)
*Figure A1: RGB LED actuator hardware used in the demonstration setup.*

### Appendix B: Firmware Listing (Single-Phase Meter via RS485)
To avoid report bloat, the full firmware source is referenced rather than embedded.

- Local template path: `plan/DeviceControlArchitecture/esp32-single-phase-energy-meter/esp32-single-phase-energy-meter.ino`
- GitHub reference: https://github.com/tharindarodrigo/lmu-iot-portal/blob/main/plan/DeviceControlArchitecture/esp32-single-phase-energy-meter/esp32-single-phase-energy-meter.ino
- Portal note: the same firmware template can be retrieved from the portal firmware template view.

![Single-phase energy meter hardware used for telemetry acquisition.](report-images/single-phase-energy.jpeg)
*Figure B1: Single-phase energy meter hardware used in the demonstration setup.*
...
- Protocol/register reference (corresponding manual sections: communication protocol and read registers):
  - ManualsLib mirror: https://www.manualslib.com/manual/3810945/Morning-Pzem-014.html (see summary sections for Page 2 and Page 4)
  - Peacefair product context: https://en.peacefair.cn/product/771.html

- Wiring/functional-block reference (corresponding manual sections: functional block diagram and wiring diagrams):
  - ManualsLib mirror: https://www.manualslib.com/manual/3810945/Morning-Pzem-014.html (see summary sections for Page 6 and Page 7)
  - Peacefair product context: https://en.peacefair.cn/product/771.html
